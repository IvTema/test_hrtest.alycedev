...
class MainPage():
    ...
    def get_all_user_apples_number(self, bttn_number):
        ...
        for i in range(self.count_user_apples(bttn_number)):
            ...
            MainPageLocators.USER_ITEMS[1] = MainPageLocators.USER_ITEMS[1][
                                             :-(len(f"[{str(bttn_number + 1)}]" + MainPageLocators.USER_APPLES[
                                                 1] + f"[{str(i + 1)}]"))]
    ...
    def refresh_list(self):
        ...
        link.click()
        time.sleep(1)
        self.all_apples_all_users = len(self.browser.find_elements(*MainPageLocators.ALL_USERS_APPLES))
        assert self.all_apples_all_users == 0, "Some users have apples after refresh"
